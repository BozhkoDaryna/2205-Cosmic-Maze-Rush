...
namespace Application.Services.UserData
{
    public class UserDataService
    {
        ...
        {
#if DEV
            _userData = _persistentDataProvider.Load<UserData>(ConstDataPath.UserDataPath,
                ConstDataPath.UserDataFileName) ?? new UserData();
#else
            _userData =
 _persistentDataProvider.Load<UserData>(ConstDataPath.UserDataPath, ConstDataPath.UserDataFileName,null, _compressor) ?? new UserData();
#endif
        }
    ...
    {
        private readonly List<MazeData> _mazeDataset;

        public MazeDataService(List<MazeData> mazeDataset)
        {
            _mazeDataset = mazeDataset;
        }

        public MazeData GetMazeData(int index)
        {
            return _mazeDataset[index];
        }
    }
}